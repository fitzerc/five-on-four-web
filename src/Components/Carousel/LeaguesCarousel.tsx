...
export function LeaguesCarousel(props: LeaguesCarouselProps) {
    const [curIdx, setCurIdx] = createSignal(0);
    const navigate = useNavigate();

    const leagueClicked = (l: League) => {
        console.log(l);
        navigate("/leagues/" + l.ID, { replace: true });
    }
    
    const previous = () => {
        if (curIdx() == 0) {
            setCurIdx(props.leagues.length - 1)
        } else {
            setCurIdx(curIdx() - 1);
        }
    };
    const forward = () => {
        if (curIdx() + 1 >= props.leagues.length) {
            setCurIdx(0);
        } else {
            setCurIdx(curIdx() + 1);
        }
    ...
    <>
        <div class="relative mx-auto max-w-2xl overflow-hidden rounded-md bg-content p-2 sm:p-4">
            <div class="absolute right-5 top-5 z-10 rounded-full bg-bkg px-2 text-center text-sm text-white">
                <span>{curIdx() + 1}</span>/<span>{props.leagues.length}</span>
            </div>
            ...
            <div class="relative h-80">
                {/* Add transitions */}
                <Index each={props.leagues}>
                    {(league, i) =>
                        <Show when={curIdx() === i}>
                            <div class="items-center justify-center flex">
                                <LeagueCard league={league()} onLeagueClick={() => leagueClicked(league())} />
                            </div>
                        </Show>
                    }
                </Index>
